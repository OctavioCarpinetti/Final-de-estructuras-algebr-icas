...
\begin{proof}
    ...
    \end{align*}
    Ahora empiezan a entrar en juego las propiedades. El valor de $x_8'$ está determinado por la orientación incial del estado. Es decir, dado que los movimientos de cara preservan la suma del vector de orientaciones,
    \begin{align*}
        0,1,2=\sum_{i=1}^8 (x_1)_i\equiv \sum_{i=1}^8 (x_2)_i=x_8'
    \end{align*}
    quedando
    \begin{align*}
        M_2(1,\tau_2,x_2,y_2)=(1,\tau_3,(0,\dots,x_8'=0,1,2),y_3)\in P
    \end{align*}
    \textbf{Paso 3.}
    ...
    Añadimos una permutación de aristas $\begin{pmatrix}1 & 2\end{pmatrix}$ para que el signo de $\tau_3$ sea positivo. $\tau_3=S_\tau S_\tau \tau_3$
    \begin{align*}
        M_3^{-1}\sigma & =1                   \\
        (M_3^{-1})_x   & =1                   \\
        M_3^{-1}\tau   & =S_\tau \circ \tau_3
    \end{align*}
    con $S_\tau =\begin{pmatrix} 1&2 \end{pmatrix}^{0,1}$. Aplicando $\left( S^{-1}\circ M_3^{-1}\right)^{-1}=M_3 \circ S$ al estado actual, obtenemos
    \begin{align*}
        M_3\circ S(1,\tau_3,(0,\dots,x_8'=0,1,2),y_3)=(1,1,(0,\dots,x_8'=0,1,2),y_4) \\
        M_3(1,\tau_3,)
    \end{align*}
...
\end{align*}
Pensemos ahora en los casos que quedan afuera. Sea $(\sigma,\tau,x,y)\in X$. Existen las posibilidades
\begin{align*}
    \sgn{\sigma}=\pm \sgn{\tau}       \\
    \sum_{i=1}^8x_i\equiv 0,1,2\mod 3 \\
    \sum_{i=1}^{12}y_i\not\equiv 0,1\mod 2
\end{align*}
mediante $\begin{pmatrix}
        1 & 2
    \end{pmatrix}$ sobre $\sigma$ de ser necesario, se puede hacer que los signos sean iguales. Mediante sumar $(0,\dots,0,+1/+2)$ sobre $x$ de ser necesario, se puede hacer que la suma sea $0\mod 3$. Y mediante sumar $(0,\dots,0/+1)$ sobre $y$ de ser necesario, se puede hacer que la suma sea $0\mod 2$.

Llamamos $T$ al movimiento que hace esto. Y notamos que es invertible. Entonces
\begin{align*}
    T(\sigma,\tau,x,y)=(\sigma',\tau',x',y')\in P=\mathcal{O}(1,1,\mathbf{0},\mathbf{0}) \\
    \implies T^{-1}\circ T(\sigma,\tau,x,y)=T^{-1}M\cdot (1,1,0,0)
\end{align*}
\end{document}
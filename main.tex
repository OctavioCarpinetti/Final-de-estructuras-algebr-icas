...
\begin{propiedad}
    Dada una configuración $(\sigma,\tau,x,y)\in X$. Sea $M\in \G$, se cumple
    \begin{align*}
        \frac{\sgn{\sigma}}{\sgn{\tau}}=\frac{\sgn{M\sigma}}{\sgn{M\tau}}
    \end{align*}
\end{propiedad}
\begin{proof}
    Sea $(\sigma,\tau,x,y)\in X$, llamamos
    \begin{align*}
        P:=\{(\sigma',\tau',x',y')\in X: \frac{\sgn{\sigma'}}{\sgn{\tau'}}=\frac{\sgn{\sigma}}{\sgn{\tau}}\}
    \end{align*}
    El estado $(\sigma,\tau,x,y)$ está en $P$ trivialmente porque $\frac{\sgn{\sigma}}{\sgn{\tau}}=\frac{\sgn{\sigma}}{\sgn{\tau}}$. Si tomamos $M_0=R,L,U,D,F,B$ (cualquiera de ellos) vemos que las componentes primera y segunda de $M_0\cdot (\sigma,\tau,x,y)$ son
    \begin{align*}
        M_0\sigma=s\circ \sigma\quad M\tau=t\circ \tau
    \end{align*}
    ...
    Tenemos que
    \begin{align*}
        \frac{\sgn{s\circ\sigma}}{\sgn{t\circ\tau}}=\frac{-\sgn{\sigma}}{-\sgn{\tau}}\implies M_0(\sigma,\tau,x,y)\in P
    \end{align*}
    concluyendo $M_0\cdot (\sigma,\tau,x,y)\in P$. Por \ref{lema:lema_de_grupos_finitos} tenemos
    \begin{align*}
        M\cdot (\sigma,\tau,x,y)\in P
    \end{align*}
    para cualquier $M\in \G$
\end{proof}
en el caso del estado resuelto tenemos
\begin{corolario}
...
\end{corolario}
\begin{proof}
    Por la propiedad anterior, tomando $(\sigma,\tau,x,y)=(1,1,\mathbf{0},\mathbf{0})$. Y considerando que $\sgn{1}=1$.
\end{proof}
Vamos con otra propiedad
\begin{propiedad}
    Dada una configuración $(\sigma,\tau,x,y)\in X$. Sea $M\in \G$, se cumple
    \begin{align*}
        \sum_{i=1}^8 M x_i\equiv \sum_{i=1}^8 x_i\mod 3
    \end{align*}
\end{propiedad}
\begin{proof}
    Sea $(\sigma,\tau,x,y)\in X$, llamamos
    \begin{align*}
        P:=\left\{(\sigma',\tau',x',y')\in X: \sum_{i=1}^8 x_i'\equiv \sum_{i=1}^8 x_i \mod 3\right\}
    \end{align*}
    el estado $(\sigma,\tau,x,y)\in P$ trivialmente porque $\sum_{i=1}^8 x_i\equiv \sum_{i=1}^8 x_i\mod{3}$. Tomamos $M_0=R,L,U,D,F,B$. Tenemos que la tercera componente de $M_0\cdot (\sigma,\tau,x,y)$, viendo la tabla \ref{tab:transformaciones_caras}, es una permutación $s$ de las componentes de $x$, sumado a un vector $k$ de suma $0$ o suma $6$. Entonces
    \begin{align*}
        \sum_{i=1}^8 M_0x_i=\sum_{i=1}^8 x_{s(i)}+\sum_{i=1}^8 k_i
    \end{align*}
    Permutar las componentes no altera la suma, y la suma de $k_i$ es $0$ o $6$ que son congruentes a $0$, de modo que
    \begin{align*}
        \sum_{i=1}^8 M_0 x_i\equiv\sum_{i=1}^8x_i\equiv 0\mod 3
    \end{align*}
    es decir $M_0\cdot (\sigma,\tau,x,y)\in P$. Que por \ref{lema:lema_de_grupos_finitos} implica
    \begin{align*}
        M\cdot (\sigma,\tau,x,y)\in P
    \end{align*}
    para cualquier $M\in \G$
\end{proof}
y para el estado resuelto tenemos
\begin{corolario}
    Dada una configuración en la órbita del estado resuelto $(\sigma,\tau,x,y)\in \mathcal{O}(1,1,\mathbf{0},\mathbf{0})$. Este cumple
    \begin{align*}
...
\end{corolario}
\begin{proof}
    Por la propiedad anterior, tomando $(\sigma,\tau,x,y)=(1,1,\mathbf{0},\mathbf{0})$. Y considerando que $\sum_{i=1}^8 0\equiv 0\mod{3}$.
\end{proof}
Y la última
\begin{propiedad}
    Dada una configuración $(\sigma,\tau,x,y)\in X$. Sea $M\in \G$, se cumple
    \begin{align*}
        \sum_{i=1}^{12} M y_i\equiv \sum_{i=1}^{12} y_i\mod 2
    \end{align*}
\end{propiedad}
\begin{proof}
    Sea $(\sigma,\tau,x,y)\in X$, llamamos
    \begin{align*}
        P:=\left\{(\sigma',\tau',x',y')\in X: \sum_{i=1}^{12} y_i'\equiv \sum_{i=1}^{12}y_i\mod 2\right\}
    \end{align*}
    el estado $(\sigma,\tau,x,y)\in P$ trivialmente porque $\sum_{i=1}^{12} y_i\equiv \sum_{i=1}^{12}y_i\mod 2$. Tomamos $M_0=R,L,U,D,F,B$. Tenemos que la cuarta componente de $M\cdot (\sigma,\tau,x,y)$, viendo la tabla \ref{tab:transformaciones_caras}, es una permutación $t$ de las componentes de $y$, sumado a un vector $k$ de suma $0$ o suma $4$. Entonces
    \begin{align*}
    ...
    \begin{align*}
        \sum_{i=1}^{12} My_i\equiv\sum_{i=1}^{12}y_i\equiv 0\mod 2
    \end{align*}
    es decir $M_0\cdot (\sigma,\tau,x,y)\in P$. Que por \ref{lema:lema_de_grupos_finitos} implica
    \begin{align*}
        M\cdot (\sigma,\tau,x,y)\in P
...
\end{corolario}
\begin{proof}
    Por la propiedad anterior, tomando $(\sigma,\tau,x,y)=(1,1,\mathbf{0},\mathbf{0})$. Y considerando que $\sum_{i=1}^{12} 0\equiv 0\mod{2}$.
\end{proof}
Estas son tres propiedades que se conservan al aplicar movimientos de caras. En particular para el estado resuelto, son propiedades de la órbita.
...
\end{propiedad}
\begin{proof}
    Sea $(\sigma,\tau,x,y)$ una configuración inicial con $\tau(e_1)=i$, $\tau(e_2)=j$ y $\tau(e_3)=k$. La estrategia es que si las tres aristas están en la misma cara paralela a la cara $M_R$. Digamos $i,j,k=2,6,7,10$ en cualquier orden.
    \begin{align*}
        i=2  & \implies U^{-1}_\tau(\tau)(e_1)=1                       \\
        i=6  & \implies R_\tau U^{-1}_\tau(\tau)R^{-1}_\tau (e_1)=1    \\
        i=7  & \implies R^{-1}_\tau U^{-1}_\tau R_\tau(\tau) (e_1)=1   \\
        i=10 & \implies R^2_\tau U^{-1}_\tau R^{2}_\tau  (\tau)(e_1)=1
    \end{align*}
    y no modifican la ubicación de $j,k$. Llamamos $(\sigma',\tau',x',y')$ al estado alcanzado tras este movimiento.
    \begin{align*}
        j=2  & \implies R_\tau F^{-1}_\tau R^{-1}_\tau(\tau')(e_2)=3  \\
        j=6  & \implies R_\tau^2 F^{-1}_\tau R^{2}_\tau(\tau')(e_2)=3 \\
        j=7  & \implies F^{-1}_\tau (\tau')(e_2)=3                    \\
        j=10 & \implies R^{-1}_\tau F^{-1}_\tau R_\tau (\tau')(e_2)=3
    \end{align*}
    y no modifican la ubicación de $k$ ni $i=1$. Llamamos $(\sigma'',\tau'',x'',y'')$ al estado alcanzado tras este movimiento.
    \begin{align*}
        k=2  & \implies R^2_\tau D^{-1}_\tau R^2_\tau(\tau'')(e_3)=11   \\
        k=6  & \implies R^{-1}_\tau D^{-1}_\tau R_\tau (\tau'')(e_3)=11 \\
        k=7  & \implies R_\tau D^{-1}_\tau R_\tau^{-1} (\tau'')(e_3)=11 \\
        k=10 & \implies D^{-1}_\tau (\tau'')(e_3)=11
    \end{align*}
    y no modifican la ubicación de $i=1$ ni $j=3$. Y el estado final tiene la propiedad que buscamos.

    Ahora, si las tres aristas no se ubican en $2,6,7,10$. Podemos simplificar cualquier estado a este caso. Digamos que el estado inicial es $(\sigma,\tau,x,y)$. Sea $\tau(e)=i$ cualquier arista que no sea $6,7,10$.
    \begin{align*}
        i=1  & \implies U_\tau(\tau)(e)=2                                 \\
        i=2  & \implies Id(\tau)(e)=2                                     \\
        i=3  & \implies U^{-1}_\tau(\tau)(e)=2                            \\
        i=4  & \implies U^2_\tau(\tau)(e)=2                               \\
        i=5  & \implies U^2_\tau L_\tau (\tau)(e)=2                       \\
        i=8  & \implies U^2_\tau L^{-1}_\tau(\tau)(e)=2                   \\
        i=9  & \implies U^2_\tau D^{-1}_\tau L^{2}_\tau D_\tau(\tau)(e)=2 \\
        i=11 & \implies U^2_\tau D_\tau L^{2}_\tau D^{-1}_\tau(\tau)(e)=2 \\
        i=12 & \implies U^2_\tau L^{2}_\tau(\tau)(e)=2
    \end{align*}
    y no modifican la ubicación de las aristas $6,7,10$.

    Si $k$ está en $6,10$ no hacemos nada. Si $k=7$ aplicamos $R^{-1}$ para llevarla a $10$. Si no es ninguno de estos, aplicamos el movimiento para llevarla a $2$, y luego aplicamos $R$ para llevarla a $6$. Tras esto $i,j,k\to i',j',k'$ con $k'=6,10$.

    Si $j'$ está en $6,7,10$ no hacemos nada. Sino aplicamos el movimiento anterior para llevarla a $2$ y luego componemos con $R$ para llevarla a $6$ y $k'$ a $7,10$. Tras esto $i,j,k\to i'',j'',k''$ con $j''=6,7,10$ y $k''=6,7,10$.

    Si $i$ está en $6,7,10$ no hacemos nada. Sino aplicamos el movimiento anterior para llevarla a $2$. Tras esto $i,j,k\to i''',j''',k'''$ con $i'''=2,6,7,10$, $j'''=6,7,10$ y $k'''=6,7,10$. Y ya estamos en el caso anterior.

    De este modo, desde cualquier estado se puede llegar a un estado en el cual las aristas $i,j,k$ estén en la posición de las aristas $2,6,7,10$. Y luego se puede llevar a la posición de las aristas $1,3,11$.
\end{proof}
\begin{corolario}
    Existe un movimiento que lleva cualesquiera tres aristas a cualesquiera otras tres aristas.
\end{corolario}
\begin{proof}
    Tomamos un movimiento $S_1$ que lleva las aristas $i,j,k$ a las aristas $1,3,11$ y otro movimiento $S_2$ que lleva las aristas $i',j',k'$ a las aristas $1,3,11$. Entonces el movimiento
    \begin{align*}
        S_{(ijk)\to (i'j'k')}:=S_2^{-1}S_1
    \end{align*}
\end{proof}
...
\begin{itemize}
    \item $M_1^{3,8}=(DRD^{-1}R^{-1}F)^3\implies M_1^{ij}=T_{ij\to3,8}([R,D]F)^3T_{ij\to 3,8}^{-1}$
...
\end{teorema}
\begin{proof}
    Como se anticipó. La estrategia consiste en reducir un estado de $P$ al estado resuelto mediante los movimientos que fuimos fabricando. Para mayor generalidad, reduciremos un elemento genérico $(\sigma,\tau,x,y)\in X$, y al final veremos qué pasa si está en $P$.

    \noindent\textbf{Paso 1.}

    Sea $(\sigma_1,\tau_1,x_1,y_1)\in X$. El movimiento $M^{ij}_1$ intercambia dos esquinas cualesquiera. Es decir que
    \begin{align*}
    ...
    \begin{align*}
        M_1(\sigma_1,\tau_1,x_1,y_1)=(1,\tau_2,x_2,y_2)
    \end{align*}
    conservando la relación entre los signos de $\sigma,\tau$, y la suma de sus orientaciones. Porque los movimientos de cara mantienen las propiedades.

    Hasta aquí, esto se puede hacer para cualquier estado sin usar ninguna de las propiedades.
    ...
    \end{align*}
    Elegimos una esquina \comillas{pivote}. Por ejemplo la $1$. Cualquier esquina $i$ de $(1,\tau_2,x_2,y_2)$ que no sea la $1$ puede orientarse aplicando $M_2^{i,1}$ una o dos veces. Esto se hace para todas las esquinas. Tras la composición queda un movimiento $M_2$ que aplicado al estado actual cumple
    \begin{align*}
        M_2(1,\tau_2,x_2,y_2)=(1,\tau_3,(x_1',0,\dots,0),y_3)
    \end{align*}
    Y el estado resultante conserva la relación de signos y la suma de orientaciones.

    Ahora empiezan a entrar en juego las propiedades. El valor de $x_1'$ está determinado por la orientación incial del estado. Dado que los movimientos de cara preservan la suma del vector de orientaciones,
    \begin{align*}
        0,1,2=\sum_{i=1}^8 (x_1)_i\equiv \sum_{i=1}^8 (x_2)_i=x_1'
    \end{align*}
    quedando
    \begin{align*}
        M_2(1,\tau_2,x_2,y_2)=(1,\tau_3,(x_1'\in(0,1,2),0\dots,0),y_3)\in P
    \end{align*}
    llamamos $x_3=(x_1'\in(0,1,2),0\dots,0)$
    ...
    \begin{align*}
        ...
        M_3^{ijk}\tau=\begin{pmatrix}
                      ...
                      \end{pmatrix}
    \end{align*}
    En general los 3-ciclos no generan cualquier permutación. Pero sí generan a las permutaciones de signo positivo. Y el signo de $\tau_3$ está determinado por la relación de signos inicial. Si $\frac{\sgn{\sigma_1}}{\sgn{\tau_1}}=1$, entonces $1=\sgn{1}=\sgn{\tau_3}$. Si $\frac{\sgn{\sigma_1}}{\sgn{\tau_1}}=-1$, entonces $-1=-\sgn{1}=\sgn{\tau_3}$.
    
    De este modo, si $\tau_3$ es de signo positivo, se escribe como una composición de 3-ciclos, y por tanto existe un movimiento $M_3^{-1}$ que cumple
    \begin{align*}
        M_3^{-1}\sigma & =1                   \\
        (M_3^{-1})_x   & =1                   \\
        M_3^{-1}\tau   & =\tau_3\\
    ...
    concluyendo
    \begin{align*}
        M_3(1,\tau_3,x_3,y_3)=(1,1,x_3,y_4)
    \end{align*}

    Si es de signo negativo, entonces consideramos
    \begin{align*}
        \begin{pmatrix}\tau_3(1)&\tau_3(2)\end{pmatrix} \circ \tau_3\circ \begin{pmatrix}
            1&2
        \end{pmatrix}=\tau_3
    \end{align*}
    y como $\begin{pmatrix} \tau_3(1)&\tau_3(2) \end{pmatrix}\circ \tau_3$ es de signo positivo, existe un movimiento $M_3^{-1}$ que cumple
    \begin{align*}
        M_3^{-1}\sigma & =1                   \\
        (M_3^{-1})_x   & =1                   \\
        M_3^{-1}\tau   & =\begin{pmatrix}
                             \tau_3(1) & \tau_3(2)
                         \end{pmatrix}\circ\tau_3\\
        \implies M_3^{-1}\tau\circ\begin{pmatrix}
            1 & 2
        \end{pmatrix}&=\tau_3
    \end{align*}
    si llamamos $S$ a la transformación que intercambia las aristas $1,2$ sin realizar ningún otro cambio, tenemos que
    \begin{align*}
        M_3^{-1}S^{-1}\sigma & =1                   \\
        (M_3^{-1}S^{-1})_x   & =1                   \\
        M_3^{-1}S^{-1}\tau   & =\tau_3
    ...
    es decir
    \begin{align*}
        S\circ M_3(1,\tau_3,x_3,y_3)=(1,1,x_3,y_4)\\
        M_3(1,\tau_3,x_3,y_3)=S^{-1}\circ(1,1,x_3,y_4)=(1,\begin{pmatrix}
            1 & 2
        \end{pmatrix},x_3,y_4)
    \end{align*}
    Teniendo entonces que existe $M_3$ tal que
    \begin{align*}
        M_3(1,\tau_3,x_3,y_3)=(1,\tau_4,x_3,y_4)
    \end{align*}
    con $\tau_4=1$ o $\tau_4=\begin{pmatrix} 1 & 2\end{pmatrix}$. Y las propiedades se conservan por ser movimientos de caras.
    ...
    \end{align*}
    Tomamos nuevamente una arista pivote. Digamos la $1$. Cada arista $i\neq 12$, con $(y_4)_i\neq 0$ puede reorientarse mediante el movimiento $M_4^{i,1}$. Tras una composición de a lo sumo $11$ movimientos, creamos un $M_4$ que cumple
    \begin{align*}
        M_4(1,\tau_4,x_3,y_4)=(1,\tau_4,x_3,(y_1'\in(0,1),0,\dots,0))
    \end{align*}
    y nuevamente, porque los movimientos preservan la suma de las orientaciones, tenemos que el $y_1'$ está determinado por la orientación inicial. Dado que los movimientos de cara preservan la suma del vector de orientaciones,
    \begin{align*}
        \sum_{i=1}^{12}y_i'=y_{12}'\equiv \sum_{i=1}^{12}y_i\equiv 0,1\mod 2\implies y_{12}'=0,1
    \end{align*}
    llamando $y_5=(y_1'\in(0,1),0,\dots,0)$
    \begin{align*}
        M_4(1,\tau_4,x_3,y_4)=(1,\tau_4,x_3,y_5)
    \end{align*}
    ...
    \begin{align*}
        M_4M_3M_2M_1(\sigma_1,\tau_1,x_1,y_1)=(1,\tau_4,x_3,y_5)
    \end{align*}
    Si se invierte el movimiento, se llega a la conclusión de que
    \begin{align*}
        M_1^{-1}M_2^{-1}M_3^{-1}M_4^{-1}(1,\tau_4,x_3,y_5)=(\sigma_1,\tau_1,x_1,y_1)\implies (\sigma_1,\tau_1,x_1,y_1)\in \mathcal{O}(1,\tau_4,x_3,y_5)
    \end{align*}
    donde $\tau_4,x_3,y_5$ son determinados por las propiedades iniciales. En particular, si $(\sigma_1,\tau_1,x_1,y_1)\in P$, entonces $\tau_4=1$, $x_3=\mathbf{0}$ y $y_5=\mathbf{0}$.
    Es decir
    \begin{align*}
...
\end{align*}
Pero la construcción del teorema también nos dice que todo estado puede llevarse a una de $12$ órbitas disjuntas entre sí. Cada una dada por una propiedad caracteristica. Estas son
\begin{align*}
    O_1&:=\mathcal{O}(1,1,(0,0,\dots,0),(0,0,\dots,0)) \\
    O_2&:=\mathcal{O}(1,1,(0,0,\dots,0),(1,0,\dots,0)) \\
    O_3&:=\mathcal{O}(1,1,(1,0,\dots,0),(0,0,\dots,0)) \\
    O_4&:=\mathcal{O}(1,1,(1,0,\dots,0),(1,0,\dots,0)) \\
    O_5&:=\mathcal{O}(1,1,(2,0,\dots,0),(0,0,\dots,0)) \\
    O_6&:=\mathcal{O}(1,1,(2,0,\dots,0),(1,0,\dots,0)) \\
    O_7&:=\mathcal{O}(1,\begin{pmatrix}
        1 & 2
    \end{pmatrix},(0,0,\dots,0),(0,0,\dots,0)) \\
    O_8&:=\mathcal{O}(1,\begin{pmatrix}
        1 & 2
    \end{pmatrix},(0,0,\dots,0),(1,0,\dots,0)) \\
    O_9&:=\mathcal{O}(1,\begin{pmatrix}
        1 & 2
    \end{pmatrix},(1,0,\dots,0),(0,0,\dots,0)) \\
    O_{10}&:=\mathcal{O}(1,\begin{pmatrix}
        1 & 2
    \end{pmatrix},(1,0,\dots,0),(1,0,\dots,0)) \\
    O_{11}&:=\mathcal{O}(1,\begin{pmatrix}
        1 & 2
    \end{pmatrix},(2,0,\dots,0),(0,0,\dots,0)) \\
    O_{12}&:=\mathcal{O}(1,\begin{pmatrix}
        1 & 2
    \end{pmatrix},(2,0,\dots,0),(1,0,\dots,0)) 
\end{align*}
y todas con el mismo número de elementos.

Las órbitas forman entonces una partición disjunta del conjunto de estados del cubo de Rubik.
\begin{align*}
    X=\bigsqcup_{i=1}^{12}O_i
\end{align*}
Y cada órbita es un conjunto de estados que no se pueden alcanzar entre sí mediante movimientos válidos del cubo. Las configuraciones iniciales de cada órbita se muestran en la figura \ref{fig:config_iniciales_orbitas}
\begin{figure}
    \begin{tikzpicture}
\matrix[
    nodes={anchor=center},
    ...
    \node{\input{Orbitas/fig1.tex}}; &
    \node{\input{Orbitas/fig2.tex}}; &
    \node{\input{Orbitas/fig3.tex}}; &
    \node{\input{Orbitas/fig4.tex}}; \\
    \node{\input{Orbitas/fig5.tex}}; &
    \node{\input{Orbitas/fig6.tex}}; &
    \node{\input{Orbitas/fig7.tex}}; &
    \node{\input{Orbitas/fig8.tex}}; \\
    \node{\input{Orbitas/fig9.tex}}; &
    \node{\input{Orbitas/fig10.tex}}; &
    \node{\input{Orbitas/fig11.tex}}; &
    \node{\input{Orbitas/fig12.tex}}; \\
};
\end{tikzpicture}
\caption{Se muestran las doce configuraciones base. Sus órbitas son una partición disjunta del espacio de configuraciones.}
\label{fig:config_iniciales_orbitas}
\end{figure}
\end{document}